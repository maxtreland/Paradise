...
/datum/space_level/proc/build_space_destination_arrays()
	...
	// Bottom border
	for(var/turf/simulated/floor/plating/asteroid/snow/naga/S in block(locate(1,1,zpos),locate(world.maxx,TRANSITION_BORDER_SOUTH,zpos)))
		transit_south |= S

	// Top border
	for(var/turf/simulated/floor/plating/asteroid/snow/naga/S in block(locate(1,world.maxy,zpos),locate(world.maxx,TRANSITION_BORDER_NORTH,zpos)))
		transit_north |= S

	// Left border
	for(var/turf/simulated/floor/plating/asteroid/snow/naga/S in block(locate(1,TRANSITION_BORDER_SOUTH + 1,zpos),locate(TRANSITION_BORDER_WEST,TRANSITION_BORDER_NORTH - 1,zpos)))
		transit_west |= S

	// Right border
	for(var/turf/simulated/floor/plating/asteroid/snow/naga/S in block(locate(TRANSITION_BORDER_EAST,TRANSITION_BORDER_SOUTH + 1,zpos),locate(world.maxx,TRANSITION_BORDER_NORTH - 1,zpos)))
		transit_east |= S

/datum/space_level/proc/add_to_transit(turf/simulated/floor/plating/asteroid/snow/naga/S)
...
/datum/space_level/proc/remove_from_transit(turf/simulated/floor/plating/asteroid/snow/naga/S)
	if(S.y <= TRANSITION_BORDER_SOUTH)
		transit_south -= S
		return
...
/datum/space_level/proc/apply_transition(turf/simulated/floor/plating/asteroid/snow/naga/S)
